...
/** BodyDetectionPlugin */
class BodyDetectionPlugin : FlutterPlugin, MethodChannel.MethodCallHandler,
    ...
    override fun onAttachedToEngine(@NonNull flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
        ...
        channel = MethodChannel(
            ...
            "com.0x48lab/body_detection"
        )
        channel.setMethodCallHandler(this)

        eventChannel = EventChannel(
            flutterPluginBinding.binaryMessenger,
            "com.0x48lab/body_detection/image_stream"
        )
    ...
    private fun handleCameraFrame(
        ...
        if ((poseDetectionEnabled || bodyMaskDetectionEnabled) && bitmap != null) {
            ...
            if (poseDetectionEnabled) {
                val processed =
                    poseDetector.process(image, OnSuccessListener { pose ->
                        eventSink?.success(
                            mapOf(
                                "type" to "pose",
                                "pose" to pose.toMap()
            ...
            if (bodyMaskDetectionEnabled) {
                val processed =
                    selfieSegmenter.process(image, OnSuccessListener { mask ->
                        eventSink?.success(
                            mapOf(
                                "type" to "mask",
                                "mask" to mask.toMap()
